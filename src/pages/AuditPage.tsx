...
${'='.repeat(analysisResults.name.length)}

OVERALL SCORE: ${analysisResults.score}/100
Business Type: ${analysisResults.businessType}
Industry: ${analysisResults.industry}
Target Audience: ${analysisResults.target_audience}
AI Ready Score: ${analysisResults.aiReadyScore}%

BRAND IDENTITY
--------------
Primary Color: ${analysisResults.primary_color}
Secondary Color: ${analysisResults.secondary_color}
Background Color: ${analysisResults.background_color}
Typography: ${analysisResults.font_style}
Brand Personality: ${analysisResults.brand_personality}
Core Values: ${analysisResults.core_values}
Key Visual Elements: ${analysisResults.key_visual_elements}

PERFORMANCE METRICS
-------------------
Color Consistency: ${analysisResults.colorConsistency}%
Visual Style Score: ${analysisResults.visualStyleScore}%
Image Quality Score: ${analysisResults.imageQualityScore}%

TOP OPPORTUNITIES
-----------------
${analysisResults.quickWin1_title ? `1. ${analysisResults.quickWin1_title}
   Current: ${analysisResults.quickWin1_current}
   Opportunity: ${analysisResults.quickWin1_opportunity}
   Impact: ${analysisResults.quickWin1_impact}
   Feature: ${analysisResults.quickWin1_feature}
` : ''}${analysisResults.quickWin2_title ? `2. ${analysisResults.quickWin2_title}
   Current: ${analysisResults.quickWin2_current}
   Opportunity: ${analysisResults.quickWin2_opportunity}
   Impact: ${analysisResults.quickWin2_impact}
   Feature: ${analysisResults.quickWin2_feature}
` : ''}${analysisResults.quickWin3_title ? `3. ${analysisResults.quickWin3_title}
   Current: ${analysisResults.quickWin3_current}
   Opportunity: ${analysisResults.quickWin3_opportunity}
   Impact: ${analysisResults.quickWin3_impact}
   Feature: ${analysisResults.quickWin3_feature}
` : ''}
BRAND ISSUES
------------
${[analysisResults.brandIssue1, analysisResults.brandIssue2, analysisResults.brandIssue3].filter(Boolean).map((issue, idx) => `${idx + 1}. ${issue}`).join('\n')}

COMPETITIVE GAPS
----------------
${[analysisResults.competitiveGap1, analysisResults.competitiveGap2, analysisResults.competitiveGap3].filter(Boolean).map((gap, idx) => `${idx + 1}. ${gap}`).join('\n')}

CONTENT MULTIPLICATION IDEAS
----------------------------
${[analysisResults.contentMultiplication1, analysisResults.contentMultiplication2, analysisResults.contentMultiplication3].filter(Boolean).map((idea, idx) => `${idx + 1}. ${idea}`).join('\n')}

SUMMARY
-------
Total Opportunities: ${analysisResults.totalOpportunities}
Analysis specialized for: ${analysisResults.industry} visual content

Generated by Dezygn Audit Tool
Visit: https://dezygn.com`;
  };
  ...
  const downloadReport = () => {
    ...
    a.href = url;
    a.download = `${analysisResults.name.replace(/[^a-zA-Z0-9]/g, '_')}_Report.txt`;
  ...
  const shareReport = () => {
    const reportText = generatePlainTextReport();
    const subject = `Website Analysis Report: ${analysisResults.name}`;
    const body = `Hi,\n\nI wanted to share this website analysis report with you:\n\n${reportText}\n\nBest regards`;
            ...
            <div className="bg-white rounded-3xl shadow-xl p-8 mb-8">
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    onClick={shareReport}
                    className="flex items-center gap-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:shadow-lg transition-all"
                  >
